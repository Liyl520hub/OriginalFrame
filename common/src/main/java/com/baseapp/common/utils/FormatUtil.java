...
public class FormatUtil {
    ...
    public static boolean isMobileNO(String mobiles) {
		 ...
		 */
        String telRegex = "[1][3456789]\\d{9}";//"[1]"代表第1位为数字1，"[3458]"代表第二位可以为3、4、5、8中的一个，"\\d{9}"代表后面是可以是0～9的数字，有9位。
     ...
     * 判断护照格式
     */
    public static boolean isPassport(String idNum){

        Pattern idNumPattern = Pattern.compile("/^[a-zA-Z0-9]{3,21}$/   /^(P\\d{7})|(G\\d{8})$/");
        //通过Pattern获得Matcher
        Matcher idNumMatcher = idNumPattern.matcher(idNum);
        if(!idNumMatcher.matches()){
            return false;
    ...
    * float类型截取至后两位小数返回
    * */
    public static String float2Str2decimal(float value){
